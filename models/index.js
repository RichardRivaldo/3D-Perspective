...
const scalerX = document.querySelector("#scale-x");
const scalerY = document.querySelector("#scale-y");
const scalerZ = document.querySelector("#scale-z");
...
function main() {
    ...
    sliderX.value = translation[0];
    sliderY.value = translation[1];
    sliderZ.value = translation[2];
    rotateX.value = radToDeg(rotation[0]);
    rotateY.value = radToDeg(rotation[1]);
    rotateZ.value = radToDeg(rotation[2]);
    scalerX.value = scale[0];
    scalerY.value = scale[1];
    scalerZ.value = scale[2];
    fieldView.value = radToDeg(fieldOfViewRadians);
    ...
    sliderZ.addEventListener("input", handleChangeZ);
    sliderZ.addEventListener("change", handleChangeZ);

    rotateX.addEventListener("input", handleRotateX);
    rotateX.addEventListener("change", handleRotateX);
    rotateY.addEventListener("input", handleRotateY);
    rotateY.addEventListener("change", handleRotateY);
    rotateZ.addEventListener("input", handleRotateZ);
    rotateZ.addEventListener("change", handleRotateZ);

    scalerX.addEventListener("input", handleScaleX);
    scalerX.addEventListener("change", handleScaleX);
    scalerY.addEventListener("input", handleScaleY);
    scalerY.addEventListener("change", handleScaleY);
    scalerZ.addEventListener("input", handleScaleZ);
    scalerZ.addEventListener("change", handleScaleZ);

    fieldView.addEventListener("input", handleFieldOfView);
    fieldView.addEventListener("change", handleFieldOfView);

    function handleFieldOfView(event) {
        const value = event.target.value;
        updateFieldOfView({ value: value });
    }
    ...
    function handleScaleZ(event) {
        const value = event.target.value;
        updateScale(2, { value: value });
    }

    function handleRotateX(event) {
        const value = event.target.value;
        updateRotation(0, { value: value });
    }

    function handleRotateY(event) {
        const value = event.target.value;
        updateRotation(1, { value: value });
    }

    function handleRotateZ(event) {
        const value = event.target.value;
        updateRotation(2, { value: value });
    }
